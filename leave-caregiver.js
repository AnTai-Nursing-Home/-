...
document.addEventListener('firebase-ready', () => {
  ...
  const SESSION_KEYS = ['antai_session_user', 'caregiverAuth', 'officeAuth', 'nutritionistAuth', 'nurseAuth'];
...
function escapeHtmlSafe(s) {
  ...
  async function loadEmployeesDropdown() {
    ...
    try {
      const [caregiversSnap, localCaregiversSnap] = await Promise.all([
      ...
      function appendEmployees(snapshot) {
      ...
      appendEmployees(caregiversSnap);
      appendEmployees(localCaregiversSnap);

      if (employeeNameSelect) employeeNameSelect.innerHTML = optionsHTML;

      autoFillFromSession();
      const currentName = employeeNameSelect?.value || '';
      if (currentName && employeeIdDisplay) {
        employeeIdDisplay.value = employeesData[currentName]?.id || '';
      }
    } catch (err) {
