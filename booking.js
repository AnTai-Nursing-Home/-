...
try { applyTranslations(); } catch(e) { console.warn('i18n apply failed:', e); }

document.addEventListener('firebase-ready', () => {

    
    // --- i18n helpers for confirmation/success display ---
    function fillOrDash(spanId, value, labelKey) {
        var span = document.getElementById(spanId);
        if (!span) return;
        var li = span.parentElement;          // <li> wrapper
        var label = span.previousElementSibling; // <strong>
        if (label && labelKey) {
            try { label.textContent = getText(labelKey); } catch(e) { label.textContent = labelKey; }
        }
