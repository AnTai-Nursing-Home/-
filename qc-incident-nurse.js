...
let currentEditingReadOnly = false;
const IS_NURSE_MODE = true;
  ...
  for (const col of collections) {
    try {
      for (const id of idCandidates) {
        const byId = await window.db.collection(col).doc(id).get();
        if (byId.exists && extractReviewPermissionFromObject(byId.data())) {
          currentUserCanReview = true;
          return true;
        }
      }
      ...
      snap.forEach(doc => {
        if (found) return;
        const d = doc.data() || {};
        const values = matchFields.map(k => String(d[k] || "").trim()).concat(String(doc.id || "").trim()).filter(Boolean);
        const matched = idCandidates.some(v => values.includes(v));
        if (matched && extractReviewPermissionFromObject(d)) {
          found = true;
          currentUserCanReview = true;
        }
      });
...
function canCurrentUserReview(d){
  return !!currentUserCanReview && !isIncidentReviewed(d);
}

function canCurrentUserEditIncident(d){
  if (!d) return false;
  const me = ensureCurrentUser();
  if (!me) return false;
  if (!IS_NURSE_MODE) return true;

  const myUid = String(me.uid || "").trim();
  const myName = normalizePersonName(me.name || "");
  const creatorUid = String(d.createdBy?.uid || "").trim();
  const creatorName = normalizePersonName(d.createdBy?.name || "");

  if (myUid && creatorUid && myUid === creatorUid) return true;
  if (myName && creatorName && myName === creatorName) return true;
  return false;
}

function canCurrentUserDeleteIncident(d){
  return canCurrentUserEditIncident(d);
}

function setIncidentFormReadOnly(readOnly, creatorName="") {
  currentEditingReadOnly = !!readOnly;
  const modal = $("modalMask");
  if (!modal) return;

  modal.querySelectorAll('input, select, textarea').forEach(el => {
    if (!el || el.id === 'editingIncidentId' || el.id === 'reviewStatusText') return;
  ...
  if (btnSave) {
    btnSave.style.display = readOnly ? 'none' : '';
    btnSave.disabled = false;
  }

  const saveHint = $("saveHint");
  if (saveHint) {
    saveHint.textContent = readOnly
      ? `此案件由 ${creatorName || '其他建立者'} 建立，您僅能查看，無法編輯或刪除。`
      : '';
  }
}
...
function setTopTitle(){
  ...
  if (currentMonth) {
    $("topTitle").textContent = "護理師｜意外事件系統";
...
async function openEditModal(docId){
  ...
  const d = snap.data() || {};
  const canEdit = canCurrentUserEditIncident(d);
  const creatorName = d.createdBy?.name || d.createdBy?.uid || "未署名";
  $("modalTitle").textContent = canEdit ? "編輯意外事件" : "查看意外事件";
  ...
  setIncidentFormReadOnly(!canEdit, creatorName);
}
...
async function renderIncidents(){
  ...
  snap.forEach(doc=>{
    ...
    const canEdit = canCurrentUserEditIncident(d);
    const canDelete = canCurrentUserDeleteIncident(d);
    const reviewedBy = d.review?.reviewedBy?.name || d.review?.reviewedBy?.uid || "";
    ...
    el.innerHTML = `
      <div class="row">
        ...
        <div class="row" style="gap:8px; justify-content:flex-end; flex-wrap:wrap;">
          ...
          <button class="btn" data-edit="${doc.id}" type="button">${canEdit ? "查看／編輯" : "查看"}</button>
          ${canReview ? `<button class="btn" data-review="${doc.id}" type="button">覆核案件</button>` : ``}
          ${!canEdit ? `<span class="tag" style="color:#64748b;">僅建立者可編輯</span>` : ``}
          ${canDelete ? `<button class="btn" data-del="${doc.id}" type="button" style="background:rgba(220,38,38,.10);border-color:rgba(220,38,38,.35);color:#7f1d1d;">刪除</button>` : `<span class="tag" style="color:#64748b;">僅建立者可刪除</span>`}
        </div>
      </div>
      <div class="metaGrid">
    ...
    if (delBtn){
      delBtn.onclick = async ()=>{
        if (!canCurrentUserDeleteIncident(d)) {
          alert("只有建立這筆案件的人才可以刪除。");
          return;
        }
...
async function saveIncident(){
  ...
  try{
    if (editingId){
      ...
      if (!canCurrentUserEditIncident(oldData)) {
        throw new Error("此案件不是您建立，僅能查看，無法修改。");
      }
...
async function init(){
  ...
  const reviewHint = $("reviewPermissionHint");
  if (reviewHint){
    reviewHint.textContent = currentUserCanReview
      ? "你目前具有覆核權限，案件建立後可在案件單按「覆核案件」，系統會自動帶入你的名字。"
      : "你目前沒有覆核權限，案件建立後僅可查看覆核狀態。";
  }

  await showMonthsView();
  initTabs();
}
