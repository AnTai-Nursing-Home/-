...
function _buildMatrixTable(items){
  ...
  // 預先建立所有基本事件類別的列（即使本期為 0）
  for (const t of IncidentTypes.concat(["未填"])) {
    if (!matrix.has(t)) matrix.set(t, new Map());
  }
  ...
  // 固定顯示所有事件類別（即使為 0），並保留資料中出現的「其他（...）」子類別
  const baseTypes = IncidentTypes.concat(["未填"]);
  const extraTypes = Array.from(matrix.keys())
    .filter(k => !baseTypes.includes(k))
    .sort((a,b)=>a.localeCompare(b,"zh-Hant"));
  const types = baseTypes.concat(extraTypes);
...
async function runStatsByRange(startDate, endDate){
  ...
  // gender（固定顯示所有類別，即使為 0）
  const gMap = _countBy(items, x=>_normalizeGender(x.residentGender));
  const gLabels = ["男","女","其他","未填"];
  const gVals = gLabels.map(k=>gMap.get(k)||0);
  ...
  // 基本類別：IncidentTypes + 未填；另外保留「其他（...）」等自訂子類別（只會在有資料時出現）
  const baseTypeLabels = IncidentTypes.concat(["未填"]);
  const extraTypeLabels = Array.from(tMap.keys())
    .filter(k => !baseTypeLabels.includes(k))
    .sort((a,b)=>a.localeCompare(b,"zh-Hant"));
  const tLabels = baseTypeLabels.concat(extraTypeLabels);
  const tVals = tLabels.map(l=>tMap.get(l)||0);

  _destroyChart("type");
  _statsCharts.type = _makeBar(document.getElementById("chartType"), tLabels, tVals);
