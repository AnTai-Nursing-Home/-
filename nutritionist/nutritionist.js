...
(function () {
  ...
  function hasNutritionistPermission(acc) {
    // ✅ 只吃你在 userAccounts 設定的欄位：
    // - canNutritionist：營養師系統權限（帳號管理新增的欄位）
    // - canOffice：辦公室管理者備援（避免把管理者鎖在外面）
    if (acc?.canNutritionist === true) return true;
    if (acc?.canOffice === true) return true;
    return false;
  }
  ...
  function boot() {
    ...
    const sess = getAuth();
    if (sess && (sess.canNutritionist === true || sess.canOffice === true)) {
      showDashboard(sess);
      return;
    }
