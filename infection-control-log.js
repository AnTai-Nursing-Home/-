(function () {
  ...
  async function exportDocx() {
    ...
    const groupDefs = [
      {
        name: 'g1',
        title: '（一）至（二）',
        rows: ROWS.filter(r => /^\(一\)|^\(二\)/.test(r.category))
      },
      {
        name: 'g2',
        title: '（三）至（四）',
    ...
    groupDefs.forEach((group, groupIndex) => {
      ...
      rows.push(new TableRow({
        ...
        children: [
          makeCell('', col1),
          makeCell('', col2),
          ...allDateCols.map(col => makeCell(col.weekday, dayWidth, AlignmentType.CENTER, 18, true))
        ]
      }));

      group.rows.forEach((row, idx) => {
        const prevCategory = idx > 0 ? group.rows[idx - 1].category : null;
        ...
        if (VerticalMergeType) {
          ...
          children.push(makeCell(isFirstOfCategory ? row.category : '', col1, AlignmentType.CENTER, 18, isFirstOfCategory));
        }
        ...
        for (const col of allDateCols) {
          ...
          children.push(makeCell(mark, dayWidth, AlignmentType.CENTER, 18, false));
        }
