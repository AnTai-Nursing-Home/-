...
(function addPeopleStats(){
  ...
  // ===== 災時員工召回名冊（依 Word 範本，橫式）=====
  (function addRecallSheets(){
    const RECALL_DATE = '20251125';
    const RECALL_HEADERS = ['序','職稱','姓名','居住地','召回時間','召回順序','手機','電話'];
    const RECALL_DATA = {
  "護理師": [
    [
      "1",
  ...
  "其他": [
    ...
    function addRecallSheet(sheetName, titleText, rows){
      ...
      ws.getCell(1,1).font = { ...fontTitle, size:22 };
      ws.getCell(1,1).alignment = {horizontal:'center', vertical:'middle'};
      ws.getRow(1).height = 26;

      // 日期列
      ws.mergeCells(2,1,2,8);
      ws.getCell(2,1).value = RECALL_DATE;
      ws.getCell(2,1).font = { name:'Microsoft JhengHei', bold:true, size:14 };
      ws.getCell(2,1).alignment = {horizontal:'center', vertical:'middle'};
      ws.getRow(2).height = 18;

      const header = ws.addRow(RECALL_HEADERS);
      styleRow(header, {isHeader:true, center:true, height:22, wrap:false});

      (rows||[]).forEach((r, idx)=>{
        const row = ws.addRow(r);
        styleRow(row, {alt:((idx+1)%2===0), height:18, wrap:false});
      });
    ...
    // 依照 Word：護理師 / 外籍照服員 / 台籍照服員 / 其他（各一頁/一分頁）
    addRecallSheet('召回名冊-護理師', '安泰醫療社團法人附設安泰護理之家災時員工召回名冊（護理師）', RECALL_DATA['護理師']);
    addRecallSheet('召回名冊-外籍照服員', '安泰醫療社團法人附設安泰護理之家災時員工召回名冊（外籍照服員）', RECALL_DATA['外籍照服員']);
    addRecallSheet('召回名冊-台籍照服員', '安泰醫療社團法人附設安泰護理之家災時員工召回名冊（台籍照服員）', RECALL_DATA['台籍照服員']);
    addRecallSheet('召回名冊-其他', '安泰醫療社團法人附設安泰護理之家災時員工召回名冊（其他）', RECALL_DATA['其他']);
  })();
;
