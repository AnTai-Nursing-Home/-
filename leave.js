document.addEventListener('DOMContentLoaded', function () {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            if (requestsByDate[dateStr]) {
                ...
                if (currentEmployee && names.includes(currentEmployee)) {
                    dayEl.classList.add('selected'); // 我預的假
                } else {
                    dayEl.classList.add('has-other-requests'); // 別人預的假
                }
    ...
    calendarDiv.addEventListener('click', (e) => {
        if (isRequestPeriodOpen) {
            ...
            if (dayEl && !dayEl.classList.contains('disabled')) {
                ...
                dayEl.classList.remove('has-other-requests'); // 點擊後，即使原本是灰色，也變為藍色
            }
        }
    });
    ...
    saveLeaveBtn.addEventListener('click', () => {
        ...
        renderCalendar(); // 儲存後重新渲染，確保姓名列表更新
    });
